# Schedulin Go Library

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Schedulin%2FGo)

The Schedulin Go library provides convenient access to the Schedulin APIs from Go.
...
```go
response, err := client.Posts.Create(...)
if err != nil {
...
```go
response, err := client.Posts.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.Posts.Create(ctx, ...)
```
...
response, err := client.Posts.Create(ctx, request, ...)
```
